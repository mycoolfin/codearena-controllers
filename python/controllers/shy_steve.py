...
def react_to_enemy(bot, my_x, my_y, my_angle, energy, enemy):
    # If an enemy is too close, run!
    dist = distance_between(my_x, my_y, enemy.x, enemy.y)
    if dist < 2:
        bot.set_speed(-ESCAPE_SPEED)
        bot.set_angle(my_angle + 30)
    elif energy > 200:  # If an enemy can't see us and we have enough energy, fight!
...
def main(bot):
    while True:
        ...
        if enemy is not None:
            react_to_enemy(bot, my_x, my_y, my_angle, energy, enemy)
            continue
